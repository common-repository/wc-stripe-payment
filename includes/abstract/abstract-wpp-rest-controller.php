 ...
 *
 */
abstract class WPP_Rest_Controller {
	...
	protected function register_authenticated_route( $route ) {
		$routes                  = get_option( 'wpp_authenticated_routes', array() );
		$route                   = '/' . trim( $route, '/' );
		$routes[ md5( $route ) ] = $route;
		update_option( 'wpp_authenticated_routes', $routes );
	}
	...
	public function rest_uri( $uri = '' ) {
		$rest_uri = stripe_wpp()->rest_api->rest_uri() . ( ! empty( $this->namespace ) ? $this->namespace : '' );
		if ( $uri ) {
	...
	public function rest_url( $uri = '' ) {
		$rest_url = stripe_wpp()->rest_api->rest_url() . ( ! empty( $this->namespace ) ? trailingslashit( $this->namespace ) : '' );
		if ( $uri ) {
	...
	public function admin_permission_check( $request ) {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			return new WP_Error( 'permission-error', __( 'You do not have permissions to access this resource.', 'wc-stripe-payments' ), array( 'status' => 403 ) );
		}
