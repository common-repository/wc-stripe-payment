...
require_once( WPP_PAYMENT_FILE_PATH . 'includes/class-wpp-payment-intent.php' );
require_once( WPP_PAYMENT_FILE_PATH . 'includes/class-wpp-payment-charge.php' );
// require_once( WPP_PAYMENT_FILE_PATH . 'includes/class-wpp-payment-charge-local.php' );

require_once( WPP_PAYMENT_FILE_PATH . 'includes/traits/wpp-payment-traits.php' );
 ...
 *
 */
abstract class WPP_Payment_Gateway_Stripe extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->hooks();
		$this->init_supports();
		$this->gateway = WPP_Stripe_Gateway::load();
	...
	public function hooks() {
		...
		add_action( 'wpp_stripe_settings_checkout_' . $this->id, array( $this, 'enqueue_admin_scripts' ) );
		add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
		add_filter( 'woocommerce_payment_methods_list_item', array( $this, 'payment_methods_list_item' ), 10, 2 );
		add_action( 'wpp_payment_token_deleted_' . $this->id, array( $this, 'delete_payment_method' ), 10, 2 );
		...
		add_filter( 'wpp_mini_cart_dependencies', array( $this, 'get_mini_cart_dependencies' ), 10, 2 );
	}
	...
	public function init_form_fields() {
		$this->form_fields = include stripe_wpp()->plugin_path() . 'includes/gateways/settings/' . str_replace( array( 'wpp_stripe_', '_' ), array(
				'',
				'-'
			), $this->id ) . '-settings.php';
		$this->form_fields = apply_filters( 'wppayment_form_fields_' . $this->id, $this->form_fields );
	}

	public function get_method_formats() {
		$class_name = 'WPP_Payment_Token_' . $this->token_type;
		if ( class_exists( $class_name ) ) {
			/**
			 *
			 * @var WPP_Payment_Token_Stripe
			 */
	...
	public function payment_fields() {
		...
		wpp_payment_token_field( $this );
		wpp_payment_intent_field( $this );
	...
	public function cart_fields() {
		...
		wpp_payment_template( 'cart/' . $this->template_name, array( 'gateway' => $this ) );
	}

	public function mini_cart_fields() {
		$this->output_display_items( 'cart' );
		wpp_payment_template( 'mini-cart/' . $this->template_name, array( 'gateway' => $this ) );
	}

	/**
	 * Enqueue scripts needed by the gateway on the frontend of the WC shop.
	 *
	 * @param WPP_Payment_Frontend_Scripts $scripts
	 */
	public function enqueue_frontend_scripts( $page = '' ) {
		global $wp;
		if ( $page ) {
			if ( 'product' === $page ) {
				$this->enqueue_product_scripts( stripe_wpp()->scripts() );
			} elseif ( 'cart' === $page ) {
				$this->enqueue_cart_scripts( stripe_wpp()->scripts() );
			} elseif ( 'checkout' === $page ) {
				$this->enqueue_checkout_scripts( stripe_wpp()->scripts() );
			} elseif ( 'mini_cart' === $page ) {
				$this->enqueue_mini_cart_scripts( stripe_wpp()->scripts() );
			} else {
				$this->enqueue_frontend_scripts();
			}
		} else {
			if ( is_add_payment_method_page() ) {
				$this->enqueue_add_payment_method_scripts( stripe_wpp()->scripts() );
			}
			if ( is_checkout() ) {
				$this->enqueue_checkout_scripts( stripe_wpp()->scripts() );
			}
		...
		if ( ! empty( stripe_wpp()->scripts()->enqueued_scripts ) ) {
			wp_enqueue_style( stripe_wpp()->scripts()->prefix . 'styles', stripe_wpp()->scripts()->assets_url( 'css/stripe.css' ), array(), stripe_wpp()->version() );
			wp_style_add_data( stripe_wpp()->scripts()->prefix . 'styles', 'rtl', 'replace' );
		}
	}
	...
	public function enqueue_mini_cart_scripts( $scripts ) {
		if ( ! wp_script_is( $scripts->get_handle( 'mini-cart' ) ) ) {
			$scripts->enqueue_script( 'mini-cart',
				...
				apply_filters( 'wpp_mini_cart_dependencies', array( $scripts->get_handle( 'wpp-payment' ) ), $scripts ) );
		}
		$scripts->localize_script( 'mini-cart', $this->get_localized_params(), 'wpp_' . $this->id . '_mini_cart_params' );
	}
	...
	public function process_payment( $order_id ) {
		...
		do_action( 'wpp_before_process_payment', $order, $this->id );
	...
	public function get_localized_params() {
		return array(
			...
			'messages'              => array(
				...
				'required_field' => __( 'Please fill out all required fields.', 'wc-stripe-payments' )
			),
			'routes'                => array(
				'setup_intent'     => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->payment_intent->rest_uri( 'setup-intent' ) ),
				'sync_intent'      => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->payment_intent->rest_uri( 'sync-payment-intent' ) ),
				'add_to_cart'      => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->cart->rest_uri( 'add-to-cart' ) ),
				'cart_calculation' => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->cart->rest_uri( 'cart-calculation' ) ),
				'shipping_method'  => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->cart->rest_uri( 'shipping-method' ) ),
				'shipping_address' => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->cart->rest_uri( 'shipping-address' ) ),
				'checkout'         => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->checkout->rest_uri( 'checkout' ) ),
				'checkout_payment' => WPP_Rest_API::get_endpoint( stripe_wpp()->rest_api->checkout->rest_uri( 'checkout/payment' ) )
			),
			'rest_nonce'            => wp_create_nonce( 'wp_rest' ),
			'banner_enabled'        => $this->banner_checkout_enabled(),
			'currency'              => get_woocommerce_currency(),
			'total_label'           => __( 'Total', 'wc-stripe-payments' ),
	...
	public function save_order_meta( $order, $charge ) {
		/**
		 *
		 * @var WPP_Payment_Token_Stripe $token
		 */
		...
		if ( wpp_is_subscription_active() && wcs_order_contains_subscription( $order ) ) {
			foreach ( wcs_get_subscriptions_for_order( $order ) as $subscription ) {
				...
				$subscription->update_meta_data( '_wpp_payment_mode', wpp_payment_mode() );
				$subscription->update_meta_data( '_wpp_payment_charge_status', $charge->status );
				$subscription->update_meta_data( '_payment_method_token', $token->get_token() );
				$subscription->update_meta_data( '_wpp_payment_customer', wpp_payment_get_customer_id( $order->get_user_id() ) );
				$subscription->save();
			}
		}
		/**
		 * @param WC_Order                  $order
		 * @param WPP_Payment_Gateway_Stripe $this
		 * @param \Stripe\Charge            $charge
		 *
		 */
		do_action( 'wpp_save_order_meta', $order, $this, $charge );
	...
	public function add_payment_method() {
		...
		try {
			if ( ! is_user_logged_in() ) {
				throw new Exception( __( 'User must be logged in.', 'wc-stripe-payments' ) );
			}

			$customer_id = wpp_payment_get_customer_id( $user_id );
			...
			do_action( 'wpp_add_payment_method_success', $result );
			...
			wc_add_notice( sprintf( __( 'Error saving payment method. Reason: %s', 'wc-stripe-payments' ), $e->getMessage() ), 'error' );
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! is_wp_error( $result ) ) {
			$order->add_order_note(
				sprintf(
					__( 'Order refunded in Stripe. Amount: %s', 'wc-stripe-payments' ),
					wc_price(
						$amount,
						array(
	...
	public function capture_charge( $amount, $order ) {
		$result = $this->gateway->charges->mode( wpp_payment_order_mode( $order ) )->retrieve( $order->get_transaction_id() );
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( ! $result->captured ) {
				...
				if ( ! is_wp_error( $result ) ) {
					...
					WPP_Utils::add_balance_transaction_to_order( $result, $order, true );
					if ( isset( $result->refunds->data[0] ) ) {
						...
						if ( ! is_wp_error( $balance_transaction ) ) {
							WPP_Utils::update_balance_transaction( $balance_transaction, $order, true );
						}
					}
					...
					$order->add_order_note( sprintf( __( 'Order amount captured in Stripe. Amount: %s', 'wc-stripe-payments' ),
							wc_price( $amount, array( 'currency' => $order->get_currency(), ) ) )
					);
				} else {
					$order->add_order_note( sprintf( __( 'Error capturing charge in Stripe. Reason: %s', 'wc-stripe-payments' ), $result->get_error_message() ) );
				}
			}
		}
	...
	public function void_charge( $order ) {
		...
		if ( is_wp_error( $result ) ) {
			$order->add_order_note( sprintf( __( 'Error voiding charge. Reason: %s', 'wc-stripe-payments' ), $result->get_error_message() ) );
		} else {
			$order->add_order_note( __( 'Charge voided in Stripe.', 'wc-stripe-payments' ) );
		}
	}
	...
	public function get_payment_token( $method_id, $method_details = null ) {
		...
		if ( class_exists( $class_name ) ) {
			...
			$token->set_environment( wpp_payment_mode() );
			if ( $method_details ) {
	...
	public function get_order_error( $error = null ) {
		wpp_payment_set_checkout_error();
		$this->last_payment_error = $error;
		do_action( 'wpp_process_payment_error', $error, $this );
	...
	public function get_payment_intent_id() {
		return ! empty( $_POST[ $this->payment_intent_key ] ) ? wc_clean( sanitize_text_field( $_POST[ $this->payment_intent_key ] ) ) : '';
	}
	...
	public function get_saved_source_id() {
		...
		if ( ! empty( $_POST[ $this->saved_method_key ] ) && ! empty( $_POST[ $this->payment_type_key ] ) && 'saved' == $_POST[ $this->payment_type_key ] ) {
			return wc_clean( sanitize_text_field($_POST[ $this->saved_method_key ]) );
		}
	...
	public function create_customer( $user_id ) {
		...
		$response = WPP_Payment_Customer_Manager::instance()->add_new_customer( $customer );
		if ( ! is_wp_error( $response ) ) {
			wpp_payment_save_customer( $response->id, $user_id );
		} else {
	 ...
	 *
	 * @return WPP_Payment_Token_Stripe|WP_Error
	 */
	 ...
	 *
	 * @param array                      $item
	 * @param WPP_Payment_Token_Stripe_CC $payment_token
	 */
	public function payment_methods_list_item( $item, $payment_token ) {
		if ( $payment_token->get_type() === $this->token_type ) {
			...
			if ( $payment_token->has_expiration() ) {
				...
				$item['expires'] = __( 'n/a', 'wc-stripe-payments' );
			}
			$item['wpp_payment_method'] = true;
		}
	 ...
	 * @param WPP_Payment_Token_Stripe $token
	 */
	...
	public function get_new_method_label() {
		return __( 'New Card', 'wc-stripe-payments' );
	}

	public function get_saved_methods_label() {
		return __( 'Saved Cards', 'wc-stripe-payments' );
	}
	...
	public function should_save_payment_method( $order ) {
		...
		if ( ! $this->use_saved_source() && ! $this->is_processing_scheduled_payment() ) {
			if ( wpp_is_subscription_active() && $this->supports( 'subscriptions' ) ) {
		 ...
		 *
		 */
		return apply_filters( 'wpp_should_save_payment_method', $bool, $order, $this );
	}
	...
	public function get_display_items( $page = 'checkout', $order = null ) {
		 ...
		 * @param string        $page
		 */
		return apply_filters( 'wpp_payment_get_display_items', $items, $order, $page );
	}
	...
	public function banner_checkout_enabled() {
		...
		return empty( $wp->query_vars['order-pay'] ) && $this->supports( 'wpp_payment_banner_checkout' ) && in_array( 'checkout_banner', $this->get_option( 'payment_sections', array() ) );
	}
	...
	public function get_update_shipping_address_response( $data ) {
		return apply_filters( 'wpp_update_shipping_address_response', $data );
	}
	...
	public function save_payment_method( $id, $order, $payment_details = null ) {
		if ( $payment_details ) {
			...
			$token->set_customer_id( wpp_payment_get_customer_id( $order->get_customer_id(), wpp_payment_order_mode( $order ) ) );
		} else {
			$token = $this->create_payment_method( $id, wpp_payment_get_customer_id( $order->get_customer_id() ) );
			if ( is_wp_error( $token ) ) {
				$this->wp_error = $token;
				$order->add_order_note( sprintf( __( 'Attempt to save payment method failed. Reason: %s', 'wc-stripe-payments' ), $token->get_error_message() ) );
	...
	public function set_payment_save_error( $order, $error ) {
		if ( wpp_is_subscription_active() && wcs_order_contains_subscription( $order ) ) {
			$message = __( 'We were not able to save your payment method. To prevent billing issues with your subscription, please add a payment method to the subscription.', 'wc-stripe-payments' );
		} else {
			$message = sprintf( __( 'We were not able to save your payment method. Reason: %s', 'wc-stripe-payments' ), $error->get_error_message() );
		}
		$order->update_meta_data( '_wpp_payment_order_error', $message );
		$order->save();
	}
	 ...
	 * @param int    $user_id
	 *
	 * @return null|WPP_Payment_Token_Stripe_CC
	 */
		...
		$payment_meta[ $this->id ] = array(
			'post_meta' => array(
				'_payment_method_token' => array(
					'value' => $this->get_order_meta_data( '_payment_method_token', $subscription ),
					'label' => __( 'Payment Method Token', 'wc-stripe-payments' ),
				),
			),
		);
	...
	public function scheduled_subscription_payment( $amount, $order ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			$order->add_order_note( sprintf( __( 'Recurring payment for order failed. Reason: %s', 'wc-stripe-payments' ), $result->get_error_message() ) );
		...
		$this->payment_method_token = $order->get_meta( '_payment_method_token' );

		if ( $result->complete_payment ) {
			if ( $result->charge->captured ) {
				WPP_Utils::add_balance_transaction_to_order( $result->charge, $order );
				$order->payment_complete( $result->charge->id );
				$order->add_order_note( sprintf( __( 'Recurring payment captured in Stripe. Payment method: %s', 'wc-stripe-payments' ), $order->get_payment_method_title() ) );
			} else {
				$order->update_status( apply_filters( 'wpp_payment_authorized_renewal_order_status', 'on-hold', $order, $this ),
					sprintf( __( 'Recurring payment authorized in Stripe. Payment method: %s', 'wc-stripe-payments' ), $order->get_payment_method_title() ) );
			}
		} else {
			$order->update_status( 'pending', sprintf( __( 'Customer must manually complete payment for payment method %s', 'wc-stripe-payments' ), $order->get_payment_method_title() ) );
		}
	}
	...
	public function is_change_payment_method_request() {
		return wpp_is_subscription_active() && did_action( 'woocommerce_subscriptions_pre_update_payment_method' );
	}
	...
	public function get_order_description( $order ) {
		return sprintf( __( 'Order %1$s from %2$s', 'wc-stripe-payments' ), $order->get_order_number(), get_bloginfo( 'name' ) );
	}
	...
	public function process_zero_total_order( $order ) {
		...
		if ( ! defined( 'processing_payment' ) && $this->should_save_payment_method( $order ) ) {
	...
	public function process_pre_order( $order ) {
		...
		if ( ! $this->use_saved_source() ) {
			...
			if ( ! $order->get_customer_id() ) {
				$customer = WPP_Payment_Customer_Manager::instance()->add_new_customer( WC()->customer );
				if ( is_wp_error( $customer ) ) {
					return wc_add_notice( $customer->get_error_message(), 'error' );
				}
				$order->update_meta_data( '_wpp_payment_customer', $customer->id );
	...
	public function save_zero_total_meta( $order, $token = null ) {
		...
		$order->update_meta_data( '_payment_method_token', $token->get_token() );
		if ( $order->get_customer_id() ) {
			$order->update_meta_data( '_wpp_payment_customer', wpp_payment_get_customer_id( $order->get_user_id() ) );
		}
		if ( wpp_is_subscription_active() && wcs_order_contains_subscription( $order ) ) {
			foreach ( wcs_get_subscriptions_for_order( $order ) as $subscription ) {
				/**
		 ...
		 *
		 */
		do_action( 'wpp_save_order_meta', $order, $this, null );
	...
	protected function order_contains_pre_order( $order ) {
		return wpp_payment_pre_orders_active() && WC_Pre_Orders_Order::order_contains_pre_order( $order );
	}
	...
	public function set_order_lock( $order ) {
		...
		set_transient( 'stripe_lock_order_' . $order_id, $order_id, apply_filters( 'wpp_set_order_lock', 2 * MINUTE_IN_SECONDS ) );
	}
	...
	public function process_pre_order_payment( $order ) {
		...
		if ( is_wp_error( $result ) ) {
			$order->update_status( 'failed' );
			$order->add_order_note( sprintf( __( 'Pre-order payment for order failed. Reason: %s', 'wc-stripe-payments' ), $result->get_error_message() ) );
		} else {
			if ( $result->complete_payment ) {
				$this->save_order_meta( $order, $result->charge );
				...
				if ( $result->charge->captured ) {
					WPP_Utils::add_balance_transaction_to_order( $result->charge, $order );
					$order->payment_complete( $result->charge->id );
					$order->add_order_note( sprintf( __( 'Pre-order payment captured in Stripe. Payment method: %s', 'wc-stripe-payments' ), $order->get_payment_method_title() ) );
				} else {
					$order->update_status( apply_filters( 'wpp_authorized_preorder_order_status', 'on-hold', $order, $this ),
						sprintf( __( 'Pre-order payment authorized in Stripe. Payment method: %s', 'wc-stripe-payments' ), $order->get_payment_method_title() ) );
				}
			} else {
				$order->update_status( 'pending', sprintf( __( 'Customer must manually complete payment for payment method %s', 'wc-stripe-payments' ), $order->get_payment_method_title() ) );
			}
	...
	public function get_order_meta_data( $meta_key, $order, $context = 'view' ) {
		...
		if ( empty( $value ) ) {
			$keys = array();
			switch ( $meta_key ) {
				case '_payment_method_token':
					$keys = array( 'wpp_payment_source_id' );
					break;
				case '_wpp_payment_customer':
					$keys = array( '_stripe_customer_id' );
					break;
				case '_payment_intent_id':
					$keys = array( '_stripe_intent_id' );
			}
			if ( $keys ) {
	...
	public function get_generic_error( $result = null ) {
		$messages = wpp_payment_get_error_messages();
		if ( isset( $messages["{$this->id}_generic"] ) ) {
			return $messages["{$this->id}_generic"];
		}

		return null != $result ? $result->get_error_message() : __( 'Cannot process payment', 'wc-stripe-payments' );
	}
	...
	public function output_display_items( $page = 'checkout', $data = array() ) {
		...
		$data  = wp_parse_args( $data, array(
			...
			'total_cents'      => wpp_payment_add_number_precision( WC()->cart->total, get_woocommerce_currency() ),
			'currency'         => get_woocommerce_currency()
		) );
		if ( in_array( $page, array( 'checkout', 'cart' ) ) ) {
			if ( ! empty( $wp->query_vars['order-pay'] ) ) {
				...
				$data['total_cents']    = wpp_payment_add_number_precision( $order->get_total(), $order->get_currency() );
		 ...
		 *
		 */
		$data = wp_json_encode( apply_filters( 'wpp_output_display_items', $data, $page, $this ) );
	...
	public function get_shipping_packages() {
		...
		if ( empty( $packages ) && wpp_is_subscription_active() && WC_Subscriptions_Cart::cart_contains_free_trial() ) {
	...
	protected function get_display_items_for_cart( $cart, $items = array() ) {
		...
		if ( $cart->needs_shipping() ) {
			...
			$items[] = $this->get_display_item_for_cart( $price, __( 'Shipping', 'wc-stripe-payments' ), 'shipping' );
		}
		...
		if ( 0 < $cart->discount_cart ) {
			...
			$items[] = $this->get_display_item_for_cart( $price, __( 'Discount', 'wc-stripe-payments' ), 'discount', $cart );
		}
		if ( ! $incl_tax && wc_tax_enabled() ) {
			$items[] = $this->get_display_item_for_cart( $cart->get_taxes_total(), __( 'Tax', 'wc-stripe-payments' ), 'tax', $cart );
		}
	...
	protected function get_display_items_for_order( $order, $items = array() ) {
		...
		if ( 0 < $order->get_shipping_total() ) {
			$items[] = $this->get_display_item_for_order( $order->get_shipping_total(), __( 'Shipping', 'wc-stripe-payments' ), $order, 'shipping' );
		}
		if ( 0 < $order->get_total_discount() ) {
			$items[] = $this->get_display_item_for_order( - 1 * $order->get_total_discount(), __( 'Discount', 'wc-stripe-payments' ), $order, 'discount' );
		}
		if ( 0 < $order->get_fees() ) {
			$fee_total = 0;
			...
			$items[] = $this->get_display_item_for_order( $fee_total, __( 'Fees', 'wc-stripe-payments' ), $order, 'fee' );
		}
	...
	protected function get_display_item_for_cart( $price, $label, $type, ...$args ) {
		return array(
			'label'   => $label,
			'pending' => false,
			'amount'  => wpp_payment_add_number_precision( $price )
		);
	}
	...
	protected function get_display_item_for_order( $price, $label, $order, $type, ...$args ) {
		return array(
			'label'   => $label,
			'pending' => false,
			'amount'  => wpp_payment_add_number_precision( $price, $order->get_currency() )
		);
	}
	...
	protected function get_display_item_for_product( $product ) {
		return array(
			...
			'pending' => true,
			'amount'  => wpp_payment_add_number_precision( $product->get_price() )
		);
	}
	...
	public function get_formatted_shipping_methods( $methods = array() ) {
		 ...
		 *
		 */
		return apply_filters( 'wpp_get_formatted_shipping_methods', $methods, $this );
	}
	...
	public function get_transaction_url( $order ) {
		if ( wpp_payment_order_mode( $order ) === 'test' ) {
	...
	protected function process_change_payment_method_request( $subscription ) {
		if ( ! $this->use_saved_source() ) {
			...
			if ( is_wp_error( $result ) ) {
				wc_add_notice( sprintf( __( 'Error saving payment method for subscription. Reason: %s', 'wc-stripe-payments' ), $result->get_error_message() ), 'error' );
		...
		$subscription->update_meta_data( '_wpp_payment_customer', $token->get_customer_id() );
		if ( $token ) {
			$subscription->set_payment_method_title( $token->get_payment_method_title() );
		}
		$subscription->save();
	...
	public function update_failing_payment_method( $subscription, $order ) {
		if ( ( $token = $this->get_token( $order->get_meta( '_payment_method_token' ), $order->get_customer_id() ) ) ) {
			$subscription->update_meta_data( '_payment_method_token', $token->get_token() );
	...
	public function get_element_options( $options = array() ) {
		...
		return apply_filters( 'wpp_stripe_get_element_options', $options, $this );
	}
