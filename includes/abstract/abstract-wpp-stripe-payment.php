...
abstract class WPP_Payment {
	...
	public function payment_complete( $order, $charge ) {
		WPP_Utils::add_balance_transaction_to_order( $charge, $order );
		$this->payment_method->save_order_meta( $order, $charge );
		if ( 'pending' === $charge->status ) {
			$order->update_status( apply_filters( 'wpp_pending_charge_status', 'on-hold', $order, $this->payment_method ),
				sprintf( __( 'Charge %1$s is pending. Payment Method: %2$s. Payment will be completed once charge.succeeded webhook received from Stripe.', 'wc-stripe-payments' ),
			...
			if ( $charge->captured ) {
				...
				$order->update_status( apply_filters( 'wpp_authorized_order_status', 'default' === $order_status ? 'on-hold' : $order_status, $order, $this->payment_method ) );
			}
			$order->add_order_note( sprintf( __( 'Order %1$s successful in Stripe. Charge: %2$s. Payment Method: %3$s', 'wc-stripe-payments' ),
				$charge->captured ? __( 'charge', 'wc-stripe-payments' ) : __( 'authorization', 'wc-stripe-payments' ),
				$order->get_transaction_id(),
				$order->get_payment_method_title() ) );
		}
		...
		/**
		 */
		do_action( 'wpp_payment_order_payment_complete', $charge, $order );
	}
	...
	public function process_refund( $order, $amount = null ) {
		...
		try {
			if ( empty( $charge ) ) {
				throw new Exception( __( 'Transaction Id cannot be empty.', 'wc-stripe-payments' ) );
			}
			 ...
			 *
			 */
			$args   = apply_filters( 'wpp_refund_args', array(
				'charge'   => $charge,
				...
				'expand'   => stripe_wpp()->advanced_settings->is_fee_enabled() ? array( 'charge.balance_transaction', 'charge.refunds.data.balance_transaction' ) : array()
			), $this, $order, $amount );
			$result = $this->gateway->refunds->mode( wpp_payment_order_mode( $order ) )->create( $args );
			if ( ! is_wp_error( $result ) ) {
				WPP_Utils::add_balance_transaction_to_order( $result->charge, $order, true );
	...
	public function process_zero_total_order( $order, $payment_method ) {
		...
		if ( 'capture' === $payment_method->get_option( 'charge_type' ) ) {
			...
			$order->update_status( apply_filters( 'wpp_authorized_order_status', 'default' === $order_status ? 'on-hold' : $order_status, $order, $payment_method ) );
		}
		WC()->cart->empty_cart();
	...
	public function add_order_metadata( &$args, $order ) {
		$meta_data = array(
			...
			'user_agent' => isset( $_SERVER['HTTP_USER_AGENT'] )  ? sanitize_text_field( $_SERVER['HTTP_USER_AGENT'] ) : 'unavailable',
			'partner'    => 'WpPayments',
			'created'    => time()
		);
		...
		$args['metadata'] = apply_filters( 'wpp_order_meta_data', $meta_data, $order );
	}
	...
	public function add_order_description( &$args, $order ) {
		$args['description'] = sprintf( __( 'Order %1$s from %2$s', 'wc-stripe-payments' ), $order->get_order_number(), get_bloginfo( 'name' ) );
	}
	...
	public function add_order_amount( &$args, $order, $amount = null ) {
		$args['amount'] = wpp_payment_add_number_precision( $amount ? $amount : $order->get_total(), $order->get_currency() );
	}
	...
	public function add_order_shipping_address( &$args, $order ) {
		if ( wpp_payment_order_has_shipping_address( $order ) ) {
			$args['shipping'] = array(
				'address' => array(
	...
	public function add_payment_failed_note( $order, $error ) {
		$note = sprintf( __( 'Error processing payment. Reason: %s', 'wc-stripe-payments' ), $error->get_error_message() );
		 ...
		 *
		 */
		$note = apply_filters( 'wpp_order_failed_note', $note, $error, $this->payment_method );
