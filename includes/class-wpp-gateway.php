...
require_once( WPP_PAYMENT_FILE_PATH . 'includes/class-wpp-api-operation.php' );
...
class WPP_Stripe_Gateway {
	...
	public static function init() {
		\Stripe\Stripe::setAppInfo( 'WordPress wp-payments', stripe_wpp()->version(), 'https://wordpress.org/plugins/wc-stripe-payments/', '{{OUR PARTNER ID}}' );
	}

	public function __get( $key ) {
		return new WPP_Payment_API_Operation( $this, $this->client, $key );
	}
	 ...
	 *
	 * @return WPP_Stripe_Gateway
	 */
	public static function load( $mode = null, $secret_key = null ) {
		$class = apply_filters( 'WPP_Stripe_Gateway_class', 'WPP_Stripe_Gateway' );
	...
	public function create_customer( $args, $mode = '' ) {
		return $this->customers->create( apply_filters( 'wpp_payment_create_customer_args', $args ), $this->get_api_options( $mode ) );
	}
	...
	public function get_api_options( $mode = '' ) {
		...
		$args = array( 'api_key' => $this->secret_key ? $this->secret_key : wpp_payment_get_secret_key( $mode ) );

		return apply_filters( 'wpp_payment_api_options', $args );
	}
	...
	private function get_error_message( $err ) {
		...
		if ( is_array( $err ) || $err instanceof \Stripe\ErrorObject ) {
			$this->messages = ! $this->messages ? wpp_payment_get_error_messages() : $this->messages;
			$keys           = array();
			if ( isset( $err['code'] ) ) {
				$keys[] = $err['code'];
		 ...
		 *
		 */
		return apply_filters( 'wpp_payment_api_request_error_message', $message, $err );
	}
