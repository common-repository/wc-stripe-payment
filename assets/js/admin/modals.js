(function ($) {
    ...
    $.fn.WPPPaymentPayOrderBackboneModal = function (options) {
        return this.each(function () {
            new $.WPPPaymentPayOrderBackboneModal($(this), options);
        })
    }

    $.WPPPaymentPayOrderBackboneModal = function (element, options) {
        var settings = $.extend({}, $.WCBackboneModal.defaultOptions, options);

        if (settings.template) {
            new $.WPPPaymentPayOrderBackboneModal.View({
    ...
    $.WPPPaymentPayOrderBackboneModal.View = $.WCBackboneModal.View.extend({
        ...
        init_card_element: function () {
            this.stripe = Stripe(wpp_payment_order_pay_params.api_key);
        ...
        api_pay_order: function () {
            this.block();
            $.ajax({
                url: wpp_payment_order_metabox_params.routes.pay,
                dataType: 'json',
                method: 'POST',
                data: $('#wpp-payment-pay-order-form').serialize(),
                beforeSend: function (xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', wpp_payment_order_metabox_params._wpnonce);
                }
        ...
        block: function () {
            this.$el.find('.wpp-backbone-modal-content').block({
                message: null,
                overlayCSS: {
                    background: '#fff',
                    opacity: 0.6
        ...
        unblock: function () {
            this.$el.find('.wpp-backbone-modal-content').unblock();
        },
